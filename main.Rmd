...
lm_forward_fit <- train(shares ~ ., data = train_df, preProcess = c("center", "scale"), method = "leapForward", tuneGrid = expand.grid(nvmax = seq(1, 55, 2)), trControl = tr_ctrl)
plot(lm_forward_fit)

min(lm_forward_fit$results$RMSE)
which.min(lm_forward_fit$results$RMSE)
```
...
min(boost_fit$results$RMSE)
which.min(boost_fit$results$RMSE)
```
...
if (length(correlated_varaibles2) > 1) { 
  corr_data2 <- train_df2 %>% select(correlated_varaibles2)
...
PC <- prcomp(select(train_df,-shares),scale = TRUE)
#Screenplot
...
#### Training
```{r}
pca_train_data <- as_tibble(predict(PC,select(train_df,-shares))) %>%
  select(PC1:PC10) %>% 
  bind_cols(select(train_df,shares))
...
pca_test_data <- as_tibble(predict(PC,select(test_df,-shares))) %>%
  select(PC1:PC10)
rfPred <- predict(rfFit, newdata = pca_test_data)
#Model Evaluation
